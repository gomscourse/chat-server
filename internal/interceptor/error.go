...
import (
	...
	"github.com/gomscourse/common/pkg/sys/validate"
...
func ErrorsInterceptor(
	...
	case sys.IsCommonError(err):
		...
		grpcCode := toGRPCCode(commEr.Code())
		err = status.Error(grpcCode, commEr.Error())
	...
	default:
		...
		if errors.As(err, &se) {
			...
			if errors.Is(err, context.DeadlineExceeded) {
				...
				fmt.Println(fmt.Sprintf("Unexpected error: %s", err.Error()))
			}
		}
